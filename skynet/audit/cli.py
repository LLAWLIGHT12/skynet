...
from skynet.audit.llm_auth import LLMAuthError, check_llm_auth

from skynet.audit.config import load_config
from skynet.audit.orchestrator import CostExceeded, run_pipeline
...
def main(ctx: click.Context, verbose: bool) -> None:
    """audit — 8-stage vulnerability discovery agent."""
...
def auth_check() -> None:
    """Verify Skynet LLM (FALLBACK_LLM_*) is configured for audit."""
    try:
        status = check_llm_auth()
    except LLMAuthError as e:
        console.print(f"[red]LLM config error:[/red] {e}")
        console.print(
            "[dim]Set FALLBACK_LLM_API_KEY, FALLBACK_LLM_API_BASE_URL, "
            "FALLBACK_LLM_MODEL_NAME (see .env.example)[/dim]"
        )
        sys.exit(2)
    console.print("[green]OK[/green] Skynet LLMClient ready")
    console.print(f"  model: {status.model_name}")
    console.print(f"  base:  {status.api_base_url}")
    console.print(f"  key:   {'configured' if status.api_key_set else 'missing'}")
...
@click.option("--graph-enhanced/--no-graph-enhanced", default=True,
              help="Enable/disable skynet code-graph enhancement.")
@click.option("--composite/--no-composite", "run_composite", default=True,
              help="Enable/disable composite vulnerability analysis.")
@click.option("--verify/--no-verify", default=False,
              help="Enable Docker sandbox PoC verification (disabled by default).")
def run(repo: str, run_id: str | None, resume: bool, max_cost_usd: float | None,
    ...
    console.print("[cyan]Skynet Audit Agent — LLMClient + tool loop (read_file/grep/glob)[/cyan]")

    config = load_config(Path(config_path)) if config_path else load_config()
    if verify:
        config.verify_enabled = True
        console.print("[cyan]Docker sandbox PoC verification enabled[/cyan]")
...
@click.option("--repo", default=None, type=click.Path(exists=True, file_okay=False),
              help="Path to the target repo (required unless using --run-id with repo path).")
@click.option("--run-id", default=None)
def status(repo: str | None, run_id: str | None) -> None:
    """Show pipeline status: tasks, findings, traces, cost."""
    if repo:
        db = StateDB.for_repo(Path(repo).resolve())
    else:
        console.print("[red]Error: --repo is required for status in skynet fusion mode[/red]")
        sys.exit(2)
    try:
        if run_id is None:
            _show_runs_table(db)
...
@main.command("report")
@click.option("--repo", default=None, type=click.Path(exists=True, file_okay=False),
              help="Path to the target repo (preferred over default state DB).")
@click.option("--run-id", required=True)
@click.option("--format", "fmt", type=click.Choice(["json", "md"]), default="json")
def report(repo: str | None, run_id: str, fmt: str) -> None:

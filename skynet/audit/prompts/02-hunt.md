...
Use JSON actions from the system prompt: `read_file`, `grep`, `glob`,
`read_node`. Target file snippets may be preloaded — read surrounding
context with `read_file` before concluding. Finish with
`{"action":"submit_final","payload":{...}}` matching `finding.schema.json`.

PoC: describe reproduction steps in `poc` fields using evidence from
`read_file`/`grep`. Live HTTP probing against `live_target` is not
available in this tool mode — rely on static proof from source code.
...
4. For each plausible finding:
   ...
   - **Attempt a PoC** (static): document reproduction in `poc.code` from
     source you read via `read_file`/`grep`. If you cannot construct a
     plausible static proof, lower severity or drop the finding.

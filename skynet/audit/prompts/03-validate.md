...
If `live_target` is present in input, note it in validation rationale
but rely on static code analysis via `read_file`/`grep` — live HTTP
probing is not available in this tool mode.

# Skynet Agent tools

`read_file`, `grep`, `glob`, `read_node`. Finish with
`{"action":"submit_final","payload":{...}}` matching
